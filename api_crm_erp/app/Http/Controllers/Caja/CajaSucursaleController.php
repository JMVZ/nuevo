...
class CajaSucursaleController extends Controller
{
    public function process_payment(Request $request)
    {
        ...
        if ($proforma_payments->count() > 0) {
            ...
            $state_payment = bccomp((string) $proforma->debt, '0.00', 2) === 0 ? 3 : 2;
        ...
        return response()->json([
            ...
            "method_payments" => $method_payments->map(function ($method_payment) {
                return [
                    ...
                    "bancos" => $method_payment->method_payments->map(function ($child) {
                        return [
                            "id" => $child->id,
                            "name" => $child->name,
                        ];
    ...
    public function search_proformas(Request $request, $client_id)
{
    ...
    $proformas = Proforma::filterShort($n_proforma, $state_payment)
        ->where("client_id", $client_id)
        ->orderBy("state_proforma", "asc")
        ->get();
    ...
    public function contract_process(Request $request)
    {
        ...
        $caja_histories = CajaHistorie::filterAdvance($n_proforma, $search_client)
            ->where("caja_sucursale_id", $caja_sucursale_id)
            ->orderBy("id", "desc")
            ->get();
    ...
    public function created_payment(Request $request)
    {
        ...
        if ($paid_out_current + $request->amount > $proforma->total) {
            return response()->json([
                "message" => 403,
                "message_text" => "El nuevo monto que se quiere ingresar supera la deuda de la proforma"
            ]);
        }
